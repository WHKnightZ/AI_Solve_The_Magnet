...
{
    for (nn = 0; nn < countEL; nn++) {
        if (EL[nn].x == c->x && EL[nn].y == c->y && EL[nn].i == c->i && EL[nn].j == c->j)
            return 1;
    }
    return 0;
}
            ...
            if (isChange) {
                ...
                if (tmpList->count == 1 && tmpList->change[0]->type == 1) {
                    if (isExist(tmpList->change[0]))
                        ...
                        EL[countEL].x = tmpList->change[0]->x;
                        EL[countEL].y = tmpList->change[0]->y;
                        EL[countEL].i = tmpList->change[0]->i;
                        EL[countEL].j = tmpList->change[0]->j;
                        countEL++;
                    }
                }
            }
...
int main()
{
    ...
    do {
        deg[currentDeg + 1].count = 0;
        //printf("%d - %d\n", currentDeg, deg[currentDeg].count);
        for (nList = 0; nList < deg[currentDeg].count; nList++) {
            ...
            if (done) {
                ...
                for (k = 0; k <= currentDeg; k++) {
                    ...
                    printf("Enter\n");
                    //printf("\n%d %d\n", deg[k].list[ListPrev[k]]->x, deg[k].list[ListPrev[k]]->y);
                }
                goToDest();
                ...
                //			    			if (deg[i].list[j]->change[k]->type==1) printf("%d %d %d %d   ",deg[i].list[j]->change[k]->x,deg[i].list[j]->change[k]->y,deg[i].list[j]->change[k]->i,deg[i].list[j]->change[k]->j); else printf("Switch  ");
                //						}
                //						printf("\n");
                //					}
                //					printf("\n");
